...
# Read the CSV file
df = pd.read_csv('mahjong-dataset/data.csv')
print(f"Found {len(df)} labeled images")
print(f"Classes: {df['class'].unique()}")

# Extract images
!unzip -q mahjong-dataset/train.zip -d mahjong-dataset/

# Create organized structure
organized_dir = './mahjong-dataset/organized/'
os.makedirs(organized_dir, exist_ok=True)

# Organize images by class
for idx, row in df.iterrows():
    image_name = row['image']
    tile_class = row['class']
    
    # Create class directory
    class_dir = os.path.join(organized_dir, str(tile_class))
    os.makedirs(class_dir, exist_ok=True)
    
    # Copy image to class directory
    src = os.path.join('mahjong-dataset/images/', image_name)
...
classes = os.listdir(organized_dir)
total_images = sum([len(os.listdir(os.path.join(organized_dir, c))) for c in classes])
print(f"✅ Organized {total_images} images into {len(classes)} classes")
